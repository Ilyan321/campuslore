...
export const TimelineSidebar: React.FC<TimelineSidebarProps> = ({
  ...
  return (
    <aside className="w-full lg:w-80 flex-shrink-0 flex flex-col blueprint-panel rounded h-[calc(100vh-5.5rem)] overflow-hidden">
      ...
      <div className="p-2 border-b border-blueprint-border bg-blueprint-surface/80">
        <div className="relative flex items-center">
          ...
          <input
            ...
            placeholder="Search topics, algorithms, keywords..."
        ...
        {searchQuery.trim() && (
          <div className="flex items-center justify-between text-[10px] font-mono text-blueprint-muted px-1 mt-1.5">
            ...
            <span className="text-blueprint-brass font-medium">{matchingWeeksCount} topic{matchingWeeksCount !== 1 ? 's' : ''} found</span>
          </div>
        )}
      </div>
          ...
          <button
            ...
            className={`w-full text-left p-2.5 rounded transition flex items-center justify-between border ${
              selectedCourseId === null && selectedWeek === null
                ...
                : 'bg-transparent border-transparent hover:bg-blueprint-raised/60 text-blueprint-secondary hover:text-blueprint-primary'
            }`}
          >
            <div className="flex items-center gap-2">
              <span className="font-mono text-[10px] font-bold uppercase tracking-wider text-blueprint-brass px-1.5 py-0.5 rounded bg-blueprint-surface border border-blueprint-border">
                AUTO
              </span>
              <div>
                <div className="text-xs font-semibold text-blueprint-primary">All Subjects (Auto-Detect)</div>
                <p className="text-[10px] font-mono text-blueprint-muted">Cross-course autonomous grounding</p>
              </div>
            </div>
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-2.5">
        {isLoading ? (
          ...
          filteredCourses.map((course) => {
            ...
            return (
              <div key={course.course_id} className="rounded border border-blueprint-border/60 bg-blueprint-surface/30 overflow-hidden">
                {/* Course Accordion Header */}
                <button
                  onClick={() => toggleCourse(course.course_id)}
                  className="w-full flex items-center justify-between px-2.5 py-1.5 text-left text-xs font-mono font-semibold text-blueprint-secondary hover:text-blueprint-primary hover:bg-blueprint-raised/50 transition border-b border-blueprint-border/40"
                >
                  <div className="flex items-center gap-2 truncate">
                    <span className="text-[10px] font-bold px-1.5 py-0.2 rounded bg-blueprint-raised border border-blueprint-border text-blueprint-primary">
                      {course.course_id}
                    </span>
                    <span className="text-blueprint-primary font-medium truncate font-sans text-xs">{course.course_name}</span>
                  </div>
                  {isExpanded ? (
                    ...
                    {course.syllabus_timeline.map((item: SyllabusWeek) => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <span className={`font-mono text-[10px] font-bold px-1 py-0.2 rounded ${isSelected ? 'bg-blueprint-brass/20 text-blueprint-brass border border-blueprint-brass/40' : 'text-blueprint-muted'}`}>
                                W{weekPadded}
                              </span>
